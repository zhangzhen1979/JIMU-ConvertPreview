        ...
        /***/ ((__unused_webpack_module, exports, __webpack_require__) => {
            ...
            const PDFViewerApplication = {
                ...
                async initialize(appConfig) {
                    ...
                    const pdfCover = this.getQuery("pdfCover");
                    if (pdfCover && pdfCover.length > 0) {
                        document.getElementById("pageRange").style.display = "block";
                ...
                ajax(type, url, parmas, callback) {
                    const xhr = new XMLHttpRequest();
                    const arr = [];
                    for (const key in parmas) {
                        if (Object.hasOwnProperty.call(parmas, key)) {
                            if (typeof parmas[key] === "string") {
                                arr.push(key + "=" + parmas[key]);
                            } else {
                                arr.push(key + "=" + JSON.stringify(parmas[key]));
                            }
                        }
                    }
                    parmas = arr.join("&");
                ...
                async save(options = {}) {
                    ...
                    try {
                        ...
                        const _that = this;
                        this.blobToDataURI(blob, function (data) {
                            const url = "/api/pdfCover";
                            const httpRequest = new XMLHttpRequest();
                            httpRequest.open("POST", url, true);
                            httpRequest.setRequestHeader("Content-type", "application/json");
                            let pageRange = document.getElementById("pageRange").value;
                            if (pageRange.length > 0) {
                                pageRange = pageRange.replaceAll("，", ",").replaceAll("-", "-");
                            }
                            const obj = {
                                uid: _that.getQuery("uid"),
                                inputFile: data,
                                pageRange
                            };
                            ...
                            httpRequest.onreadystatechange = function () {
                                if (httpRequest.readyState === 4 && httpRequest.status === 200) {
                                    ...
                                    console.log(json);
                                    const respData = JSON.parse(json);
